...
class Level {
    ...
    createEnemies() {
        const enemyPositions = [
            { x: 22 * GAME_CONSTANTS.TILE_SIZE, y: 22 * GAME_CONSTANTS.TILE_SIZE, type: ENEMY_TYPES.GOOMBA },
            { x: 40 * GAME_CONSTANTS.TILE_SIZE, y: 22 * GAME_CONSTANTS.TILE_SIZE, type: ENEMY_TYPES.GOOMBA },
            { x: 53 * GAME_CONSTANTS.TILE_SIZE, y: 22 * GAME_CONSTANTS.TILE_SIZE, type: ENEMY_TYPES.GOOMBA },
            { x: 54 * GAME_CONSTANTS.TILE_SIZE, y: 22 * GAME_CONSTANTS.TILE_SIZE, type: ENEMY_TYPES.GOOMBA },
            { x: 82 * GAME_CONSTANTS.TILE_SIZE, y: 22 * GAME_CONSTANTS.TILE_SIZE, type: ENEMY_TYPES.GOOMBA },
            { x: 99 * GAME_CONSTANTS.TILE_SIZE, y: 22 * GAME_CONSTANTS.TILE_SIZE, type: ENEMY_TYPES.KOOPA_TROOPA },
            { x: 125 * GAME_CONSTANTS.TILE_SIZE, y: 22 * GAME_CONSTANTS.TILE_SIZE, type: ENEMY_TYPES.GOOMBA },
            { x: 142 * GAME_CONSTANTS.TILE_SIZE, y: 22 * GAME_CONSTANTS.TILE_SIZE, type: ENEMY_TYPES.GOOMBA },
            { x: 143 * GAME_CONSTANTS.TILE_SIZE, y: 22 * GAME_CONSTANTS.TILE_SIZE, type: ENEMY_TYPES.GOOMBA },
            { x: 175 * GAME_CONSTANTS.TILE_SIZE, y: 22 * GAME_CONSTANTS.TILE_SIZE, type: ENEMY_TYPES.GOOMBA },
            { x: 176 * GAME_CONSTANTS.TILE_SIZE, y: 22 * GAME_CONSTANTS.TILE_SIZE, type: ENEMY_TYPES.GOOMBA }
        ];
    ...
    createBricks() {
        const brickPositions = [
            // 最初のブロック群
            { x: 21, y: 20 }, { x: 24, y: 20 }, { x: 25, y: 20 }, { x: 26, y: 20 }, { x: 27, y: 20 },
            { x: 21, y: 16 }, { x: 24, y: 16 }, { x: 25, y: 16 }, { x: 26, y: 16 }, { x: 27, y: 16 },

            // 中間のブロック群
            { x: 80, y: 20 }, { x: 81, y: 20 }, { x: 83, y: 20 }, { x: 84, y: 20 }, { x: 87, y: 20 },
            { x: 91, y: 20 }, { x: 92, y: 20 }, { x: 93, y: 20 },

            // 後半のブロック群
            { x: 101, y: 20 }, { x: 119, y: 20 }, { x: 121, y: 20 }, { x: 122, y: 20 }, { x: 123, y: 20 },
            { x: 128, y: 20 }, { x: 131, y: 20 }, { x: 132, y: 20 },

            // 上層のブロック
            { x: 110, y: 16 }, { x: 111, y: 16 }, { x: 113, y: 16 }, { x: 128, y: 16 }, { x: 131, y: 16 },
            { x: 132, y: 16 }, { x: 168, y: 20 }, { x: 170, y: 20 }
        ];
    ...
    createBackground() {
        ...
        const cloudPositions = [
            { x: 8, y: 4 }, { x: 19, y: 4 }, { x: 27, y: 12 }, { x: 36, y: 4 },
            { x: 56, y: 4 }, { x: 72, y: 12 }, { x: 88, y: 4 }, { x: 104, y: 4 }
        ];

        // 丘の配置
        const hillPositions = [
            { x: 0, y: 23 }, { x: 48, y: 23 }, { x: 96, y: 23 }, { x: 144, y: 23 }
        ];

        // 茂みの配置
        const bushPositions = [
            { x: 11, y: 25 }, { x: 41, y: 25 }, { x: 59, y: 25 }, { x: 75, y: 25 }
        ];
    ...
    getEntitiesInRange(cameraX, cameraWidth) {
        ...
        return {
            blocks: this.blocks.filter(block =>
                block.x + block.width > leftBound && block.x < rightBound
            ),
            enemies: this.enemies.filter(enemy =>
                enemy.active && enemy.x + enemy.width > leftBound && enemy.x < rightBound
            ),
            items: this.spawnedItems.filter(item =>
